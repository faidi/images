...
public class Database {
	...
	public void chargeIMGInFolder(String name, String location)
			throws Exception {
	...
	/**
	 * cette fnct permet de trouver les images similaire a une image données la
	 * comparaison se fait une fois la signature de l'image en paramétre est
	 * calculer. la fnct charge toutes les signatures depuis la base et
	...
	public ArrayList<ImgToWeb> trouverSimilaire(ObjetImage img)
			throws SQLException {
						...
						do {
							...
							if (dist != 0) {
								...
								try {
									...
									imageListName.add(imagesResultat(rs
											.getString(1)));
	...
	public ImgToWeb imagesResultat(String name) throws SQLException {
		...
		try {
			Connection conn = getConnection();
			PreparedStatement ps = conn
					.prepareStatement("select name,img from image where name=?");
			...
			try {
				...
				try {
					if (rs.next()) {
						...
						img = new ImgToWeb(rs.getBinaryStream(2),
								rs.getString(1));
